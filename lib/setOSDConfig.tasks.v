  ...
  begin
    case (videomode)
      ...
      `USE_FXD60: begin
          cfg_osd_vscale <= 3'b001;
          cfg_osd_hscale <= 2'b00;
          cfg_osd_voffset <= 90;  // (`VSYNCLEN_720p60 + `VBACKPORCH_720p60 + (`VACTIVE_720P60 - 2*`OSD_WINDOW_VACTIVE)/2)/2;
                                  // = (5 + 20 + (720 - 2*205)/2)/2 = 90
          cfg_osd_hoffset <= 701; // (`HSYNCLEN_720p60 + `HBACKPORCH_720p60 + (`HACTIVE_720P60 - `OSD_WINDOW_HACTIVE)/2)/1
                                  // = (40 + 220 + (1280 - 399)/2)/1 = 701
         end
      `USE_720p60: begin
...
//         end
      ...
      `USE_FXD50: begin
          cfg_osd_vscale <= 3'b001;
          cfg_osd_hscale <= 2'b00;
          cfg_osd_voffset <= 90;  // (`VSYNCLEN_720p50 + `VBACKPORCH_720p50 + (`VACTIVE_720P50 - 2*`OSD_WINDOW_VACTIVE)/2)/2;
                                  // = (5 + 20 + (720 - 2*205)/2)/2 = 90
          cfg_osd_hoffset <= 701; // (`HSYNCLEN_720p50 + `HBACKPORCH_720p50 + (`HACTIVE_720P50 - `OSD_WINDOW_HACTIVE)/2)/1
                                  // = (40 + 220 + (1280 - 399)/2)/1 = 701
         end
      `USE_720p50: begin
          cfg_osd_vscale <= 3'b010;
