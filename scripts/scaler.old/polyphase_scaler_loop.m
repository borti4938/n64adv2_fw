...
scaling_loop = 1:0.0625:6;
scaling_loop = 1:0.125:7;
scaling_loop = [1:0.125:3.5 3.75:0.25:6.0];
factor_bits = 8;

method = 0; % 0 = nearest neighbor, 1 = pixel-growing, 2 = bilinear
...
fileName_phases = 'output_phases.txt';
fileName_filter_descr = 'output_filter_desc.txt';
fileName_verilog = 'output_verilog_cases.txt';

%% calculation stuff

fid0 = fopen(fileName_phases,'w');
fid1 = fopen(fileName_filter_descr,'w');
fid2 = fopen(fileName_verilog,'w');
fid_cnt = 0;

for scaling = scaling_loop
    ...
    scaler.mem_elem = zeros(2,pixel_out.num);
    scaler.fir_a = zeros(2,pixel_out.num);
    ...
    if (print_results)
        ...
        if (print_verilog)
            if (valid_0a && valid_0b && valid_1a && valid_1b)
                ...
                verilog_load_pattern_main = '32''b';
...
%                 verilog_fir_a0_bypass_main = [verilog_fir_a0_bypass_main '''b'];
                verilog_fir_a0_bypass_main = '32''b';;
                for idx= 0:pattern_length-1
        ...
        if (pattern_length_main <= 32)
            fprintf(fid0,['  {' int2str(length(init_load)-1) ',' ...
                int2str(length(post_load)-1) '}, // ' ...
                int2str(floor(scaling)) '.' ...
                sprintf('%03d',round((scaling-floor(scaling))*1000)) 'x\n']);
            if (method == 0)
                main_a0_init_hex = '80';
            end
            if (strcmp(main_a0_incr_hex,'0'))
                main_a0_incr_hex = '00';
                main_a0_additional_increment_bin = '32''b0';
            end
            fprintf(fid1,['  {0x' main_a0_init_hex ',' ...
                int2str(init_bypass(end)) ',' ...
                '0x' main_a0_incr_hex ',' ...
                int2str(pattern_length_main-1) ',' ...
                '0b' verilog_load_pattern_main(5:end) ',' ...
                '0b' main_a0_additional_increment_bin(5:end) ',' ...
                '0b' verilog_fir_a0_bypass_main(5:end) '}, // ' ...
                int2str(floor(scaling)) '.' ...
                sprintf('%03d',round((scaling-floor(scaling))*1000)) 'x\n']);
            
            fprintf(fid2,[ '  ' int2str(fid_cnt) ': begin //'...
                int2str(floor(scaling)) '.' sprintf('%03d',round((scaling-floor(scaling))*1000)) 'x\n'... 
                '      phase_length_init <= 3''d' int2str(length(init_load)-1) ';\n'...
                '      pattern_length_main <= 6''d' int2str(pattern_length_main-1) ';\n'...
                '      phase_length_post <= 3''d' int2str(length(post_load)-1) ';\n'...
                '      a0_last_init <= 8''h' main_a0_init_hex ';\n'...
                '      z0_last_init <= 1''b' int2str(init_bypass(end)) ';\n'...
                '      load_pattern_main <= 32''b' verilog_load_pattern_main(5:end) ';\n'...
                '      a0_increment <= 8''h' main_a0_incr_hex ';\n'...
                '      a0_round_inc <= 32''b' main_a0_additional_increment_bin(5:end) ';\n'...
                '      z0_bypass_z0_pattern_main <= 32''b' verilog_fir_a0_bypass_main(5:end) ';\n'...
                '    end\n']);
            fid_cnt = fid_cnt + 1;
        end
